        ...
        select_lex->first_cond_optimization)                          // 11
    {
      ...
      if (!in_subs->is_registered_semijoin)
      {
        ...
        OPT_TRACE_TRANSFORM(writer, oto0, oto1, select_lex->select_number,
                            "IN (SELECT)", "semijoin");
        oto1.add_member("chosen").add_bool(true);
      }
    }
    else
    {
...
bool subquery_types_allow_materialization(THD* thd, Item_in_subselect *in_subs)
{
  ...
  for (uint i= 0; i < elements; i++)
  {
    ...
    if (!inner->type_handler()->subquery_type_allows_materialization(inner,
                                                                     outer))
    {
      oto1.add_member("possible").add_bool(false);
      oto1.add_member("cause").add_str("types_mismatch");
      DBUG_RETURN(FALSE);
    }
  }
  ...
  */
  ...
  else
  {
    ...
    oto1.add_member("sjm_scan_allowed").add_bool(all_are_fields);
    oto1.add_member("possible").add_bool(true);
    DBUG_PRINT("info",("subquery_types_allow_materialization: ok, allowed"));
    DBUG_RETURN(TRUE);
  }
  oto1.add_member("possible").add_bool(false);
  oto1.add_member("cause").add_str(cause);
  DBUG_RETURN(FALSE);
}
  ...
  while ((in_subq= li++))
  {
    ...
    if (in_subq->do_not_convert_to_sj)
    {
      OPT_TRACE_TRANSFORM(writer, oto0, oto1,
                          ...
                          "IN (SELECT)", "semijoin");
      oto1.add_member("converted_to_semi_join").add_bool(false);
      oto1.add_member("cause").add_str("subquery_attached_to_the_ON_clause");
      break;
    }
    ...
    if (in_subq->is_flattenable_semijoin) 
    {
      ...
      {
        oto1.add_member("converted_to_semi_join").add_bool(false);
        oto1.add_member("cause")
            .add_str("table_in_parent_join_now_exceeds_MAX_TABLES");
        break;
      }
      if (convert_subq_to_sj(join, in_subq))
        goto restore_arena_and_fail;
      oto1.add_member("converted_to_semi_join").add_bool(true);
    }
    else
    {
