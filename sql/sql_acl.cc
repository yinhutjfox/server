...
class User_table_json: public User_table
{
  ...
  int get_auth(THD *thd, MEM_ROOT *root, ACL_USER *u) const
  {
    ...
    if (get_value(STRING_WITH_LEN("auth_or"), JSV_ARRAY, &array, &array_len))
    {
      u->alloc_auth(root, 1);
  ...
  {
    const char *authstr= get_str_value(root, STRING_WITH_LEN("authentication_string"));
    const char *plugin= get_str_value(root, STRING_WITH_LEN("plugin"));
    if (plugin && authstr)
    {
      if (plugin && *plugin)
      {
  ...
  bool set_auth(const ACL_USER &u) const
  {
    ...
    json.append(']');
    return set_value(STRING_WITH_LEN("auth_or"), json.ptr(), json.length(), false) == JSV_BAD_JSON;
  }
  bool set_auth1(const ACL_USER &u, uint i) const
  {
    return set_str_value(STRING_WITH_LEN("plugin"),
                         u.auth[i].plugin.str, u.auth[i].plugin.length) ||
            set_str_value(STRING_WITH_LEN("authentication_string"),
 ...
 private:
  ...
  enum json_types set_value(const char *key, size_t klen,
  ...
  bool set_str_value(const char *key, size_t klen, const char *val, size_t vlen) const
  {
    ...
    if (blen < 0)
      return 1;
    return set_value(key, klen, buf, blen, true) == JSV_BAD_JSON;
  }
  bool set_int_value(const char *key, size_t klen, longlong val) const
  {
    ...
    return set_value(key, klen, v, vlen, false) == JSV_BAD_JSON;
  }
  ...
  {
    return set_value(key, klen, val ? "true" : "false", val ? 4 : 5, false) == JSV_BAD_JSON;
  }
};
